...
function setView($root, $target, $route, $param = [], $default = 'not-found')
{
    ...
    if (view()->exists($root . '.pages.' . $target)) {
        ...
        if ($default == 'not-found') {
            $param = ['message' => 'File halaman tidak ditemukan "' . $target . '"'];
        }
    }
...
function getRoleUser($roleName, $access = '')
{
    if (session()->has('userData')) {
        ...
        if (is_string($sessionRole['roleValue'])) {
            if ($sessionRole['roleValue'] == 'superadmin') {
                return true;
            }
...
function getSite($key, $default = '', $media = false)
{
    if ($media) {
        if (is_array($key)) {
            $data = \App\Models\Master\Setting::where('key', $key[0])->where('type', $key[1])->where('value', '!=', '')->with(['media'])->first();
        } else {
            $data = \App\Models\Master\Setting::where('key', $key)->where('value', '!=', '')->with(['media'])->first();
        }

        if ($data && $data->media) {
            $default = $data->media->path;
        }
    } else {
        if (is_array($key)) {
            $data = \App\Models\Master\Setting::where('key', $key[0])->where('type', $key[1])->where('value', '!=', '')->first();
        } else {
            $data = \App\Models\Master\Setting::where('key', $key)->where('value', '!=', '')->first();
        }
...
function setTransType($title, $key = 'transaction_type_code')
{
    $data = \DB::table('transaction_types')->where('transaction_type_name', $title)->first();
    if ($data) {
        return $data->{$key};
    }
...
function getCoaFromSubCategory($subCategoryId, $itemTypeId, $coaType)
{
    //coa_type = [persediaan,biaya,tax]
    $optionCoaType = [
        'persediaan' => 'inventory_coa_id',
        'biaya'      => 'expense_coa_id',
        'tax'        => 'tax_coa_id',
    ];

    if (! isset($optionCoaType[strtolower($coaType)])) {
        return 0;
    }

    $data = DB::table('item_account_settings')
        ->where('sub_category_id', $subCategoryId)
        ->where('item_type_id', $itemTypeId)
        ->value($optionCoaType[strtolower($coaType)]);
    return $data;
}
...
{
    $defaultLang = session('lang') ? session('lang') : 'id';
    $data        = App\Models\Master\Setting::where('key', $key)->where('type', 'lang')->where('value', '!=', '')->first();
    if ($data && $defaultLang) {
    ...
    return is_string($default) ? strtoupper($default) : $default;
}

function getMultiLangContent($key, $array = [], $default = '')
{
    $defaultLang = session('lang') ? session('lang') : 'id';
    $data        = App\Models\Master\Setting::where('key', $key)->where('type', 'lang')->where('value', '!=', '')->first();
    if ($data && $defaultLang) {
...
function getMultiLangFull($key, $array = [], $default = '')
{
    $data = App\Models\Master\Setting::where('key', $key)->where('type', 'lang')->where('value', '!=', '')->first();
    if ($data) {
        $exp = (array) json_decode($data->value);
        if (count($exp) == 2) {
            $default = $exp['id'] . ' / <i>' . $exp['en'] . '</i>';
        }
    }
...
function val_exist_with_old($object, $key, $isUpper = true)
{
    $res = '';
    if ($object != '' && isset($object->{$key})) {
        $res = is_string($object->{$key}) ? ($isUpper ? strtoupper($object->{$key}) : $object->{$key}) : $object->{$key};
    }

    if (old($key)) {
        $res = is_string(old($key)) ? strtoupper(old($key)) : old($key);
    }

    return $res;
}

function val_exist_with_type($object, $relationKey, $key)
{
    $res       = '';
    $tempKey   = substr($key, 0, -2);
    $tempIndex = substr($key, -1);
    if ($object != '' && isset($object->{$relationKey})) {
        $newObj = [];
        $newObj = $object->{$relationKey}->where('type', $tempKey)->pluck('value');
        $res    = isset($newObj[$tempIndex]) ? $newObj[$tempIndex] : '';
    }
...
function val_exist_with_multi_data($object, $relationKey, $key)
{
    $res       = '';
    $tempKey   = substr($key, 0, -2);
    $tempIndex = substr($key, -1);
    if ($object != '' && isset($object->{$relationKey})) {
        $newObj = [];
        $newObj = $object->{$relationKey};
        $res    = isset($newObj[$tempIndex]) ? $newObj[$tempIndex]->{$tempKey} : '';
    }

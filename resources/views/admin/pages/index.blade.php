    ...
    <div class="page-header-content">
        ...
        <div class="heading-elements">
            @if (getRoleUser(request()->route()->getName(),
                    'create') && in_array('create', getAttributPage($menu, $type, 'activity')))
            ...
            <div class="panel-heading element-filter" style="display: none;">
                <div class="row">
                    ...
                    @foreach (array_slice($cols['table'], 1) as $klab => $lab)
                        ...
                        @if (${$lab}['type'] == 'select')
                            @if (isset(${$lab}['route']))
                                <div class="col-md-3" style="margin-bottom:10px;height:91px;">
...
@include('admin.components.modal-refund')
@include('admin.components.modal-rewash')

@if (!request()->ajax())
    @push('scripts')
    @endif
    <script src="{{ asset('packages/datatables/datatables.min.js') }}"></script>
    <script src="{{ asset('packages/datatables/extensions/fixed_header.min.js') }}"></script>
    <script>
        var type = "{{ $type }}";
        ...
        var branch_id = "{{ in_array(session('userData')['roleId'], [1]) }}" ? '' : localStorage.getItem('magicOutlate');
        var editable = "{{ getRoleUser($getMenu, 'edit') }}";
        var removable = "{{ getRoleUser($getMenu, 'delete') }}";
        var traceable = "{{ getRoleUser($getMenu, 'trace') }}";
        var otherAccess = {!! getRoleUser($getMenu) !!};

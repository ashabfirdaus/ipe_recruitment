            ...
            <div class="category-content">
                <div class="media">
                    ...
                    <div class="media-body">
                        ...
                        <div class="text-size-mini text-muted" id="header-outlate">
                    ...
                    @foreach ($menus as $menu)
                        @if ($menu['route'] == 'newsection')
                            <li class="navigation-header">
                                <span>{{ $menu['label'] }}</span> <i class="icon-menu" title="{{ $menu['label'] }}"></i>
                            </li>
                        @else
                            ...
                            @else
                                <li class="parent-tag-menu">
                                    <a href="javascript:void(0)">
                                        <i class="{{ $menu['icon'] ? $menu['icon'] : 'icon-stack2' }}"></i>
                                        ...
                                        @foreach ($menu['submenu'] as $submenu)
                                            <li class="{{ activeMenu($submenu, request()->route()->getName()) }}"
                                                ...
                                                <a href="{{ empty($submenu['param']) ? route($submenu['route']) : route($submenu['route'], $submenu['param']) }}"
                                                    class="me">
                                                    <span
                                                        id="menu-{{ $submenu['route'] }}">{{ $submenu['label'] }}</span>
                                                </a>
                                            </li>
                                        @endforeach
                                    </ul>
